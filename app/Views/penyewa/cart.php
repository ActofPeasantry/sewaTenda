...
<script>
    ...
    function checkInputValue(input, tendaId) {
        ...
        tendaList = tendaList.map(tenda =>{
				if(tenda.id == tendaId){
					return {...tenda, jumlahHari :  input.value };
				}
        ...
        if(tendaList!=null && tendaList.length>0){
            if(tendaList.some(value => value.jumlahHari != 0)){
                $.ajax({
                    ...
                    data: { tendaList: JSON.stringify(tendaList), alamat: document.getElementById('inputAlamat').value, tanggalMulaiSewa: document.getElementById('inputTanggalMulaiSewa').value},
